...
QUESTION = (
    "A supplier breached a contract by failing to deliver unique goods on time. "
    "What remedies and case law may apply?"
)
...
AGENT_PROMPT = (
    SYSTEM_PROMPT
    + " For contract breach questions, also use search_case_law to identify relevant precedent."
)
...
async def main():
    ...
    llm = get_llm()
    graph = create_react_agent(model=llm, tools=TOOLS, prompt=AGENT_PROMPT)
