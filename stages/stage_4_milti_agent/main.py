...
async def check_routing(state: LegalState) -> dict:
    """Keyword router: determine which specialist agents are needed without an LLM call."""
    print("\n  [Node: check_routing] Determining which specialists are needed...")
...
async def privacy_agent(state: LegalState) -> dict:
    ...
    print("\n  [Node: privacy_agent] Privacy/GDPR specialist starting...")
    llm = get_llm()
...
def draw_graph():
    """Display the compiled graph in a notebook, or print Mermaid text in a terminal."""
    graph = create_graph()
    try:
